# customized pipline('fill-mask') can deal with multiple <masks>
...
class FillMaskPipeline(Pipeline):
    ...
    def __call__(self, *args, targets=None, top_k: Optional[int] = None, **kwargs):
        ...
        # assert batch_size == 1, 'multiple masks only support batch_size=1'
        ...
        for i in range(batch_size):
            ...
            for num_mask in range(num_masks):
                tmp_result = []
                for v, p in zip(values[num_mask].tolist(), predictions[num_mask].tolist()):
                    tokens = input_ids.clone().numpy()
                    tmp_masked_index = masked_index[num_mask]
                    tokens[tmp_masked_index] = p
                    # Filter padding out:
